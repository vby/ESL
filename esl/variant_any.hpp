...
private:
	...
	template <class T>
	using alternative_index_t = typename alternative_index<T>::type;
...
public:
	...
	template <std::size_t I, class... Args, class T = alternative_type_t<I>>
	explicit variant_any(std::in_place_index_t<I>, Args&&... args): any(std::in_place_type<T>, std::forward<Args>(args)...), index_(I) {}

	template <std::size_t I, class U, class... Args, class T = alternative_type_t<I>>
...
inline constexpr bool holds_alternative(const ::esl::variant_any<Ts...>& va) noexcept {
	return va.index() == ::esl::types_index_v<decay_t<T>, Ts...>;
}
...
template <class T, class... Ts, class = enable_if_t<::esl::types_count_v<T, Ts...> == 1>>
