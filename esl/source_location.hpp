...
#    define ESL_HAS_SOURCE_LOCATION
        const char* file = __builtin_FILE(), const char* func = __builtin_FUNCTION(), std::uint_least32_t line = __builtin_LINE(),
        std::uint_least32_t column = 0
#else
        const char* file, const char* func, std::uint_least32_t line, std::uint_least32_t column = 0
#endif
        ) noexcept {
        source_location sl;
        sl.file_ = file;
