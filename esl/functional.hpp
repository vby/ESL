...
inline std::size_t hash_value(const T& value, std::size_t h) noexcept(noexcept(std::declval<std::hash<T>>()(std::declval<const T&>()))) {
	return hash_combine(h, hash_value(value));
}
// stdhash
...
#define ESL_FF_(a, b, c, d, M, s, t) a = (a + (d ^ (b & (c ^ d))) + M + t); a = rotl(a, s) + b;
#define ESL_GG_(a, b, c, d, M, s, t) a = (a + (c ^ (d & (c ^ b))) + M + t); a = rotl(a, s) + b;
#define ESL_HH_(a, b, c, d, M, s, t) a = (a + (b ^ c ^ d) + M + t); a = rotl(a, s) + b;
#define ESL_II_(a, b, c, d, M, s, t) a = (a + (c ^ (b | (~d))) + M + t); a = rotl(a, s) + b;
